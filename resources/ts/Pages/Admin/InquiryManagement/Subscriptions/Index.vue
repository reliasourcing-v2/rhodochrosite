<template>
    <admin-layout title="Subscription">
        <header-section
        ...
        <Tabs
            ...
            :tab-route="route('admin.contact.subscriptions.index')"
        />
        <page-template>
            <div class="px-6 py-4">
                <Filter
                                ...
                                <tr class="h-20">
                                    ...
                                    >
                                        <template v-if="selectedTab !== 'archived'">
                                            <edit-button
                                                :routeLink="route('admin.contact.subscriptions.view', item.id)"
                                            />
                                            <delete-button
                                                v-if="selectedTab !== 'archived'"
                                                :modal-title="`Archive Subscription #${item.id}`"
                                                :modal-name="`${item.id}`"
                                                :route-link="route('admin.contact.subscriptions.delete', item.id)"
                                            />
                                        </template>
...
const breadcrumbs = [
    ...
    {
        title: 'Index',
        route: route('admin.contact.subscriptions.index')
    }
]
...
const processArchive = (): void => {
    router.delete(
        route('admin.contact.subscriptions.delete', selectedItem.value.id),
        {
            preserveState: false
        }
    )
...
const processRestore =(): void => {
    router.post(
        route("admin.contact.subscriptions.restore"),
