<template>
    <admin-layout title="Articles">
        ...
        <Tabs
            ...
            :tab-route="route('admin.article-management.articles.index')"
        >
            <template #buttons>
                <create-button
                    :routeLink="route('admin.article-management.articles.create')"
                />
            </template>
        </Tabs>
        <page-template>
                                ...
                                <tr class="h-20">
                                    ...
                                    >
                                        <template v-if="selectedTab !== 'archived'">
                                            <edit-button
                                                class="mr-3"
                                                :routeLink="route('admin.article-management.articles.edit', item.id)"
                                            />
                                            <delete-button
                                                :modal-title="`Archive Article #${item.id}`"
                                                :modal-name="`Article #${item.id}`"
                                                :route-link="route('admin.article-management.articles.archive', item.id)"
                                            />
                                        </template>
...
const breadcrumbs = [
    ...
    {
        title: 'Index',
        route: route('admin.article-management.articles.index')
    }
]
